...
public class FriendFollowingScript : MonoBehaviour
{
    ...
    {
        GameObject[] followings = GameObject.FindGameObjectsWithTag("FriendFollowing");
    ...
    public void followingscene_client_to_server()
    {
        ...
        following_scene.uid = socketpp.other_player_uid;
        socketpp.receiveMsg = socketpp.socket(JsonUtility.ToJson(following_scene));//현재 다른 사용자 uid 클라이언트에서 서버로 전달
    ...
    public void MakeFollowing(int following_uid, string following_nickname, string following_profile_timestamp, int following_profile_size)
    {
        if (!Directory.Exists(Application.persistentDataPath + "/" + following_uid.ToString()))
            Directory.CreateDirectory(Application.persistentDataPath + "/" + following_uid.ToString() + "/"); //플레이어 uid 폴더 없을 시 생성
        ...
        clone_following_friend_button.GetComponent<Button>().onClick.AddListener(() => GameObject.Find("MasterCanvas").GetComponent<MainSceneScript>().ActiveFriendPage()); //팔로잉 페이지 들어가기 버튼 생성
        GameObject clone_following_friend_deletebutton = clone_following_friend.transform.Find("deleteButton").gameObject;
        clone_following_friend_deletebutton.SetActive(false); //팔로잉 삭제 버튼 끄기
