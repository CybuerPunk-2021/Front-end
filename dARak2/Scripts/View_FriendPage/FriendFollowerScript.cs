...
public class FriendFollowerScript : MonoBehaviour
{
    ...
    public void UpdateFollower()
    {
        ...
        {
            Destroy(follower); //follower Prefab 전부 제거
        }
        followscene_client_to_server(); //follower 가져오기
    }
    ...
    public void followscene_client_to_server()
    {
        ...
        socketpp.receiveMsg = socketpp.socket(JsonUtility.ToJson(follow_scene));//현재 다른 사용자 uid 클라이언트에서 서버로 전달
        ...
        for (int i = 0; i < 50; i++)
        {
            MakeFollower(followers.follower[i].uid, followers.follower[i].nickname, timedict[followers.follower[i].uid], sizedict[followers.follower[i].uid]); //팔로워 정보로 팔로워 Prefab생성
        }
    }

    //팔로워 Prefab생성
    public void MakeFollower(int follower_uid, string follower_nickname, string follower_profile_timestamp, int follower_profile_size)
    {
        if (!Directory.Exists(Application.persistentDataPath + "/" + follower_uid.ToString()))
            Directory.CreateDirectory(Application.persistentDataPath + "/" + follower_uid.ToString() + "/"); //플레이어 uid 폴더 없을 시 생성
        ...
        clone_follower_friend.GetComponent<PrefabUid>().uid = follower_uid; //팔로워 ujid.
        ...
        clone_follower_friend_button.GetComponent<Button>().onClick.AddListener(() => GameObject.Find("MasterCanvas").GetComponent<MainSceneScript>().ActiveFriendPage()); //팔로워 페이지 들어가기 버튼 생성
        GameObject clone_follower_friend_addbutton = clone_follower_friend.transform.Find("addButton").gameObject;
        clone_follower_friend_addbutton.SetActive(false); //팔로워 추가 버튼 끄기
