...
public class AlbumScript : MonoBehaviour
{
    ...
    public int album_snapshot_count = 0; //스냅샷 리스트 번호
    Socketpp socketpp;
    ...
    public void EraseAlbum()
    {
        ...
        album_snapshot_count = 0; //리스트 번호 0으로 초기화
    }
    ...
    public void album_client_to_server()
    {
        ...
        if (!Directory.Exists(Application.persistentDataPath + "/" + socketpp.player_uid.ToString()))
            Directory.CreateDirectory(Application.persistentDataPath + "/" + socketpp.player_uid.ToString() + "/"); //플레이어 uid 폴더 없을 시 생성
        ...
        album_snapshot_count++; //리스트 번호 증가
        for (int i = 0; i < 4; i++)
        {
    ...
    public void MakeClone(string snapshot_text, int snapshot_like, string snapshot_timestamp, int snapshot_size)
    {
        ...
        clone_albumImage.GetComponent<PrefabUid>().uid = socketpp.player_uid; //앨범이미지 uid = 사용자 uid
        clone_albumImage.GetComponent<PrefabUid>().nickname = socketpp.player_nickname; //앨범이미지 닉네임 = 사용자 닉네임
        ...
        clone_albumImage_btn.GetComponent<Button>().onClick.AddListener(() => GameObject.Find("MasterCanvas").GetComponent<MainSceneScript>().ActiveEditSnapshotPage()); //앨범 이미지 수정 버튼 추가
    }
}
