...
public class MyPageScript : MonoBehaviour
{
    ...
    public void profile_client_to_server()
    {
        ...
        if (!Directory.Exists(Application.persistentDataPath + "/" + socketpp.player_uid.ToString()))
            Directory.CreateDirectory(Application.persistentDataPath + "/" + socketpp.player_uid.ToString() + "/"); //플레이어uid폴더 없을 시 생성
        ...
        checkprofile.uid = new int[1] { socketpp.player_uid };
        ...
        socketpp.player_profile_size = checkprofiletime.size[0];

        if (checkprofiletime.timestamp[0] != "")
        {
            string profile_path = socketpp.player_uid.ToString() + "/" + socketpp.player_uid.ToString() + "_" + checkprofiletime.timestamp[0] + ".png";
    ...
    public void loadvisitbook_client_to_server()
    {
        ...
        socketpp.receiveMsg = socketpp.socket(JsonUtility.ToJson(loadvisitbook)); //사용자uid 방명록 리스트 숫자 클라이언트에서 서버로 전달
        ...
        for (int i = 0; i < 5; i++)
        {
            MakePost(visitbook.visit_book[i].writer_uid, visitbook.visit_book[i].nickname, visitbook.visit_book[i].comment, visitbook.visit_book[i].timestamp, timedict[visitbook.visit_book[i].writer_uid], sizedict[visitbook.visit_book[i].writer_uid]);//방명록 정보로 방명록 Prefab생성
        }
    }
    ...
    public void MakePost(int visitbook_uid, string visitbook_nickname, string visitbook_comment, string visitbook_timestamp, string visitbook_writer_timestamp, int visitbook_writer_size)
    {
        ...
        post_profile_image.GetComponent<Button>().onClick.AddListener(() => GameObject.Find("MasterCanvas").GetComponent<MainSceneScript>().ActivePostToFriendPage()); //방명록 이미지 클릭시 페이지로 이동
